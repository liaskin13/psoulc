...
function MembersPanel({ onClose, viewer = "D" }) {
  ...
  const handleAdd = (e) => {
    ...
    const planet =
      newTier === "C"
        ? newMoonName.trim()
          ? `${LOCKBOX_PREFIX}${newMoonName.trim().toLowerCase()}`
          : null
        : newPlanet || null;
      ...
      <div className="inbox-panel-body">
        ...
        {flashCode && (
          <div className="inbox-code-flash" style={{ marginBottom: "12px" }}>
            <span className="inbox-code-label">
              {flashCode.name} — PERSONAL CODE
            </span>
            <span className="inbox-code-value">{flashCode.code}</span>
            <span className="inbox-code-sub">TRANSMIT TO MEMBER</span>
            <button
              className="inbox-action-btn inbox-action-cancel"
              onClick={() => setFlashCode(null)}
              style={{ marginTop: "8px" }}
            >
              DISMISS
            </button>
        ...
        {tab === "members" && (
          <>
                ...
                <div key={m.id} className="members-card">
                  ...
                  <div className="members-card-meta">
                    {m.tier === "C" && m.planet?.startsWith(LOCKBOX_PREFIX) ? (
                      ...
                      <span className="members-planet-badge">
                        {m.planet.toUpperCase()}
                      </span>
                    ) : (
                      <span className="members-planet-badge members-planet-open">
                        OPEN ACCESS
                      </span>
                    )}
                    <span
            ...
            {!showAdd ? (
              <button
                className="inbox-action-btn inbox-action-approve members-add-btn"
              ...
              <form className="members-add-form" onSubmit={handleAdd}>
                ...
                {newTier === "B" ? (
                  ...
                  <input
                    ...
                    value={newMoonName}
                    onChange={(e) =>
                      setNewMoonName(e.target.value.toUpperCase())
                    }
                    maxLength={32}
                    required
                  />
        ...
        {tab === "listeners" &&
          (listeners.length === 0 ? (
            <div className="inbox-empty">
              <div className="inbox-empty-glyph">◎</div>
              <div className="inbox-empty-text">NO GUESTS</div>
            </div>
          ) : (
            listeners.map((l) => (
              <div key={l.id} className="members-card">
                ...
                <div className="members-card-meta">
                  ...
                  <span className="members-created-by">
                    {new Date(l.joinedAt).toLocaleDateString("en-US", {
                      month: "short",
                      day: "numeric",
                    })}
                  </span>
