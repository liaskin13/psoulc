...
function trackToCell(t) {
  const parsedHotCues = t.hot_cues
    ? typeof t.hot_cues === "string"
      ? JSON.parse(t.hot_cues)
      : t.hot_cues
    : {};
  ...
  return {
    ...
    metadata: {
      ...
      waveformData: t.waveform_data
        ? typeof t.waveform_data === "string"
          ? JSON.parse(t.waveform_data)
          : t.waveform_data
        : null,
...
function TheVault({ vault, onBack, onExitSystem, onVoid, readOnly = false }) {
  ...
  const handleReanalyze = async () => {
    ...
    try {
      const { waveformData } = await reanalyzeFromUrl(
        url,
  ...
  const displayItems = (() => {
    ...
    if (sortBy === "bpm") {
      ...
      sorted.sort(
        (a, b) => (a.metadata?.duration ?? 0) - (b.metadata?.duration ?? 0),
      );
          ...
          {activeId && (
            ...
            >
              <div className="vault-deck-waveform-meta">
                <span className="vault-deck-time">
                  {formatTime(currentTime)}
                </span>
                <span className="vault-deck-title">
                  {activeCell?.label}
                  {activeCell?.metadata?.musicalKey && (
                    <span className="vault-deck-key">
                      {" "}
                      · {activeCell.metadata.musicalKey}
                    </span>
                  )}
                </span>
                <span className="vault-deck-duration">
                  {formatTime(duration || activeCell?.metadata?.duration)}
                </span>
              </div>
              <DeckWaveform
                ...
                {[1, 2, 3, 4, 5, 6, 7, 8].map((num) => {
                  ...
                  return (
                    <button
                      ...
                      title={
                        cue
                          ? `CUE ${num} @ ${formatTime(cue.time)} — click to jump · right-click to clear`
                          : `Set CUE ${num} at ${formatTime(currentTime)}`
                      }
                      aria-label={cue ? `Jump to cue ${num}` : `Set cue ${num}`}
                    >
                      <span className="vault-cue-num">{num}</span>
                      {cue && (
                        <span className="vault-cue-time">
                          {formatTime(cue.time)}
                        </span>
                      )}
                    </button>
          ...
          <div className="shelf-section">
            <div className="vault-library-controls">
              ...
              <div className="vault-track-count" aria-live="polite">
                {loading
                  ? "—"
                  : `${displayItems.length}${searchQuery ? `/${trackItems.length}` : ""}`}
              </div>
            </div>
            {!loading && (
              <RecordShelf
