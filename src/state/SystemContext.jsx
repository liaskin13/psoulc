...
function loadListeners() {
  try {
    return JSON.parse(localStorage.getItem(LISTENERS_KEY)) || [];
  } catch (_) {
    return [];
  }
}
...
function loadTuneOverrides() {
  try {
    return JSON.parse(localStorage.getItem(TUNE_OVERRIDES_KEY)) || {};
  } catch (_) {
    return {};
  }
}
...
export function SystemProvider({ children }) {
  ...
  const [collaborators, setCollaborators] = useState(() =>
    loadCollaborators(loadMembers(), loadListeners()),
  );
  ...
  const [commandLog, setCommandLog] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("psc_command_log")) || [];
    } catch (_) {
      return [];
    }
  });
  ...
  useEffect(() => {
    try {
      localStorage.setItem(REGISTRY_KEY, JSON.stringify(architectArchive));
    } catch (_) {}
  }, [architectArchive]);

  useEffect(() => {
    try {
      localStorage.setItem(INBOX_KEY, JSON.stringify(inboxRequests));
    } catch (_) {}
  }, [inboxRequests]);
  ...
  useEffect(() => {
    try {
      localStorage.setItem(LISTENERS_KEY, JSON.stringify(listeners));
    } catch (_) {}
  }, [listeners]);
  ...
  useEffect(() => {
    try {
      localStorage.setItem(TUNE_OVERRIDES_KEY, JSON.stringify(tuneOverrides));
    } catch (_) {}
  }, [tuneOverrides]);
  ...
  const restoreItem = (itemId) => {
    setArchitectArchive((prev) =>
      prev.map((item) =>
        item.id === itemId
          ? { ...item, restored: true, restoredAt: new Date().toISOString() }
          : item,
      ),
    );
  ...
  const finalApproveRequest = (id, planet = null) => {
    ...
    if (req) {
      ...
      const collab = makeCollaborator({
        ...
        role: "editor",
        grantedBy: "D",
        vaultAccess: planet ? [planet] : [],
        code,
        planet,
      });
      setCollaborators((prev) => [
        ...prev,
        { ...collab, legacyId: newMember.id },
      ]);
    }
    return code;
  };
  ...
  const unreadCount = inboxRequests.filter(
    (r) => !r.read && r.status === "approved_L",
  ).length; // D sees approved_L
  const unreadCountL = inboxRequests.filter(
    (r) => !r.read && r.status === "pending_L",
  ).length; // L sees pending_L
  ...
  const addMember = (
    ...
    // D7: create matching collaborator record
    const role =
      tier === "A" ? "co-owner" : tier === "C" ? "featured-artist" : "editor";
    const collab = makeCollaborator({
      name,
      role,
      grantedBy: createdBy,
  ...
  const revokeCollaborator = (collaboratorId) => {
    setCollaborators((prev) =>
      prev.map((c) =>
        c.id === collaboratorId ? { ...c, isActive: false } : c,
      ),
    );
  };
  ...
  const getCollaboratorForSession = (meta = sessionMeta) => {
    ...
    return (
      collaborators.find(
        (c) =>
          isCollaboratorActive(c) &&
          (c.name === meta.owner || c.planet === meta.vault),
      ) ?? null
    );
  };
  ...
  const canEnterVault = (meta, vaultId) => {
    ...
    return meta.tier === "B" && meta.vault === vaultId;
  };
    ...
    (cmd, payload = {}) => {
      ...
      const collab =
        collaborators.find(
          (c) =>
            isCollaboratorActive(c) &&
            (c.name === sessionMeta?.owner || c.planet === sessionMeta?.vault),
        ) ?? null;
      if (
        collab &&
        payload.vaultId &&
        !canCollaboratorAccess(collab, payload.vaultId)
      ) {
        console.warn(
          `[PSC] CMD "${cmd}" denied — collaborator lacks access to vault "${payload.vaultId}"`,
        );
      ...
      const handlers = {
        [CMD.VOID_ITEM]: () => {
          if (payload.item && payload.vaultId) {
            voidItem(payload.item, payload.vaultId);
            return payload.item;
          }
        },
        [CMD.RESTORE_ITEM]: () => {
          if (payload.id) {
            restoreItem(payload.id);
            return payload.id;
          }
        },
        ...
        [CMD.TUNE_VAULT]: () => {
          if (payload.vaultId && payload.itemId) {
            saveTuneOverride(payload.vaultId, payload.itemId, payload.override);
            return payload;
          }
        },
      ...
      setCommandLog((prev) => {
        ...
        try {
          localStorage.setItem("psc_command_log", JSON.stringify(next));
        } catch (_) {}
        return next;
      });
